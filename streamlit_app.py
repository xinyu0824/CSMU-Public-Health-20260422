    ...
    if not st.session_state.login:
                ...
                for i, url in enumerate(p_urls):
                    if url.strip() != "":
                        with cols[i % 3]:
                            ...
                            label = t_names[i] if i < len(t_names) else ""
                            st.caption(label)
        ...
        with tab1:
            ...
            if st.session_state.locked_task:
                ...
                up_file = st.file_uploader("上傳證物", type=['png', 'jpg', 'jpeg'], key="agent_upload")
                if up_file:
                    if st.button("🚀 正式回傳總部"):
                        with st.spinner("同步中..."):
                            try:
                                ...
                                try:
                                    ...
                                    val = int(float(raw_val)) if (pd.notna(raw_val) and str(raw_val) != "") else 0
                                except: val = 0
